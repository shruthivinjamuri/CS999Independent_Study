...
public class Graph implements IGraph {
	...
	public String getPath(String from, String to, String pathAlgo) {
		...
		switch (pathAlgo) {
			case "Dijkstra":  prev = dijkstraShortestPath(source);
							 break;
			case "BellmanFord": prev = bellmanFordShortestPath(source);
							 break;
			default: prev = dijkstraShortestPath(source);
					 break;
		}
		
		ArrayList<IEdge> path = new ArrayList<>();
		INode target = destination;
		if(destination.getMinCost() == Double.MAX_VALUE || prev == null){
			return "<false />";
		}
	...
	public HashMap<INode, IEdge> dijkstraShortestPath(INode source) {
		...
		for (INode node : getAllNodes()) {
			if(!node.equals(source)) node.setMinCost(Double.MAX_VALUE);
			prev.put(node, null);
			queue.add(node);
		}
